...
int bmpLoad(BMP *bmp, char *fileName) {
   ...
   if(bmp->height > 0)
       size = bmp->width * bmp->height * bmp->bitsPerPixel / 8;
   else 
       size = bmp->width * (-bmp->height) * bmp->bitsPerPixel / 8;
   
   bmp->data = (BYTE *)malloc(size);
   fseek(file, bmp->dataOffset, SEEK_SET);
   len = fread(bmp->data, 1, size, file);
   printf("size = %u ret=%u\n", size, len);
...
int main(int argc, char *argv[]) {
    ...
    //bmpLoad(&bmp, argv[1]);
    bmpLoad(&bmp, "test.bmp");
    bmpPrint(&bmp);
    //bmpSetBox(&bmp, 0, 0, 20, 20, 0x66, 0x33, 0x99);
    //bmpSave(&bmp, "test2.bmp");
    return 0;
}
